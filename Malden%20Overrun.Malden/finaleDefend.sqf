...
while{FinalePhase == 3} do {
	...
	if(_westUnits == 0 && _eastUnits >= 1) then {
		...
		if (_zombieControl == false) then {
			...
			//warn players
			[["finaleDefend",300,"We have to clear out those zombies or the Djinn will kill us!"],"messageNear.sqf"] remoteExec ["BIS_fnc_execVM",0];			
		...
		if(_loopCount == _defendTime) then {
			[["finaleDefend",300,"THE DJINN IS ANGRY"],"messageNear.sqf"] remoteExec ["BIS_fnc_execVM",0];
